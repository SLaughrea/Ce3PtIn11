...
def getErrorBinA(f,b,binRange,binT,binA,filename,alpha):
    ...
    # calculate bins of f and b
        ...
        errB[i] = np.sqrt(1+varB)
        errF[i] = np.sqrt(1+varF)
...
for filename in filenames:    
    ...
    #a = ((1-alpha)+a*(1+alpha))/((1+alpha)+a*(1-alpha))
    
    #a = ((alpha-1)+a*(1+alpha))/(alpha*beta+1)+a*(alpha*beta-1)
    
    #a = ((1-alpha)+a*(1+alpha*beta))/(1+alpha)+a*(1-alpha*beta)
    ...
    errorBinA=getErrorBinA(f,b,binRange,t,a,filename,alpha)
    ...
    ranges[1] = [1,1.2,0.2]
    ranges[2] = [0.001,0.02,0.02-0.001]
    ranges[3] = [0,0.01,0.01]
    ranges[4] = [0.0,2,2]
    ranges[5] = [120,150,30]
    ranges[6] = [0,0.01,0.01]
    ranges[7] = [0.,0.8,0.8]
    ranges[8] = [20,50,30]
    ...
    # comparing cumulative evidence from different models hints at the model probability
    peaks_table_SD5, peaks_error_table_SD5, peaks_rel_height_table_SD5=posterior_peaks(posterior_pdf_SD5,showResults=True)
...
#    mean_SD5, cov_SD5 = dyfunc.mean_and_cov(samples_SD5, weights_SD5)
    ...
    #bestPeaks=[0,0,0,0,0,0,0,0,0]
    #plotResults("005813",binT,binA,errorBinA,bestPeaks)
    plotResults("005813",t,a,errorBinA,bestPeaks)
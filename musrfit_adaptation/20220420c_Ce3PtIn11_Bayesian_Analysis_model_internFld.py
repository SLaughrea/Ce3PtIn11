...
​
    ...
    return fig, axes
#%% traceplot modifications needed
...
def getErrorBinA(f,b,binT,binA,alpha):
...
errorBin = getErrorBinA(f, b, binT, binA, alpha)
...
n = 5000
#asy1Guess, asy2Guess, asyBkgd, beta1, beta2, field1, field2, sigma, lambdaL, lambdaT1, lambdaT2  = vector
testVector = [0.0326,0.0343,0.156,0.303,0.69,20,8,0.013,0.003,0.68,0.0034]
#optimized  [0.031197869942756418,0.036423202644933886,0.1578531424477351,0.35621347945063875,0.7097989026258776,20.007078094866014,7.987505374336176,0.011499726023460631,0.004509852962415846,0.6830081337350468,0.0037177255323765186]
...
SsamplerDyn= dynesty.DynamicNestedSampler(log_likelihood, prior_transform, ndim=ndim)
SsamplerDyn.run_nested()
sresults = SsamplerDyn.results
from dynesty import plotting as dyplot

# determined by trial and error
truths = [0.0425,0.043,0.155,0.295,0.52,20,7.98,0.004,0.001,0.682,0.001]
...
internFld_cumulative_evidence = sresults["logz"][-1]
...
for i in range(len(mean)):
    ...
    print(opt)
print("log_z posterior evidence = {:}".format(internFld_cumulative_evidence))
...
for i in range(len(posterior_pdf)):
    ...
    peaks, _ = signal.find_peaks(y,height=0.0001)
    if len(peaks) == 0:
        peaks = [0]
...
fig, axes = plt.subplots(11, figsize=(10, 7), sharex=True)
samples = sampler.get_chain(discard=100,thin=15)
labels = ["signal_1_asymmetry", "signal_2_asymmetry", "bkgd_asymmetry", "amplitude_1", "amplitude_2", "field_1", "field_2", "$\sigma$", "$\lambda_L$", "$\lambda_{T1}$", "$\lambda_{T2}$"]
for i in range(ndim):
    ax = axes[i]
    ax.plot(samples[:, :, i], "k", alpha=0.3)
    ax.set_xlim(0, len(samples))
...
# comparing cumulative evidence from different models hints at the model probability
internBsl_cumulative_evidence = sresults["logz"][-1]
from dynesty import utils as dyfunc
...
for i in range(len(mean)):
    opt = "{:20} : {:25} ± {:25}".format(labels[i],mean[i],cov[i,i])
    print(opt)
print("log_z posterior evidence = {:}".format(internBsl_cumulative_evidence))

...
def log_prior(vector):
    a1, a2, aB, b1, b2, p1, p2, f1, f2, sg, l1, l2, t1, t2 = vector
    if (a1min < a1 < a1max and a2min < a2 < a2max and aBmin < aB < aBmax and b1min < b1 < b1max and \
        ...
        l1min < l1 < l1max and l2min < l2 < l2max and t1min < t1 < t1max and t2min < t2 < t2max ):
        return 0
    return -np.inf
...
def prior_transform(vector):
    ...
    return np.array([vector[0]*a1range+a1min,vector[1]*a2range+a2min,vector[2]*aBrange+aBmin,vector[3]*b1range+b1min, \
                     vector[4]*b2range+b2min,vector[5]*p1range+p1min,vector[6]*p2range+p2min,vector[7]*f1range+f1min,vector[8]*f2range+f2min,vector[9]*sgrange+sgmin, \
                     vector[10]*l1range+l1min,vector[11]*l2range+l2min,vector[12]*t1range+t1min,vector[13]*t2range+t2min])
...
def modelInternFld(vector):
    asy1Guess, asy2Guess, asyBkgd, beta1, beta2, phi1, phi2, field1, field2, sigma, lambdaL1, lambdaL2, lambdaT1, lambdaT2  = vector
    ...
    internFld_signal1 = (beta1*cosFunction1*np.exp(-lambdaT1*binT)+(1-beta1)*np.exp(-lambdaL1*binT))
    internFld_signal2 = (beta2*cosFunction2*np.exp(-lambdaT2*binT)+(1-beta2)*np.exp(-lambdaL2*binT))
...
def modelInternBsl(vector):
    ...
    internBsl_signal1 = (beta1*besselFunction1*np.exp(-lambdaT1*binT)+(1-beta1)*np.exp(-lambdaL1*binT))
    internBsl_signal2 = (beta2*besselFunction2*np.exp(-lambdaT2*binT)+(1-beta2)*np.exp(-lambdaL2*binT))
...
alpha = 1.06323 # variable based on run
total_asymmetry = 0.07 # variable based on run
...
labels = ["signal_1_asymmetry", "signal_2_asymmetry", "bkgd_asymmetry", "amplitude_1", "amplitude_2", "phi_1", "phi2", "field_1", "field_2", "sigma", "lambdaL1", "lambdaL2" , "lambdaT_1", "lambdaT_2"]
ndim = len(labels)
# range estimations
...
p1min=0.;p1max=360.;p1range=p1max-p1min
p2min=0.;p2max=360.;p2range=p2max-p2min
f1min=0.;f1max=70.;f1range=f1max-f1min
f2min=0.;f2max=70.;f2range=f2max-f2min
sgmin =0.;sgmax =0.1 ;sgrange=sgmax-sgmin
l1min=0.;l1max=0.2  ;l1range=l1max-l1min
l2min=0.;l2max=0.2  ;l2range=l1max-l1min

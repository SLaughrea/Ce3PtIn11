...
def plotResults(filename,goodBinT,goodBinA,goodErrorBinA,vector):
    ...
    plt.plot(goodBinT, model(vector), label="fit",color="orange",linewidth=4)
    title = "Asymmetry fit for run " + filename
    plt.xlabel("time (s)",fontsize=12)
    plt.ylabel("Asymmetry",fontsize=12)      
    plt.yticks(ticks=[0.15,0.16,0.17,0.18,0.19,0.20,0.21,0.22,0.23,0.24],labels=[0.15,0.16,0.17,0.18,0.19,0.20,0.21,0.22,0.23,0.24])
...
"""
# 3 et 4 PARTAGENT ASY?
# NEED TO MODIFY LIMITS
def model(vector):
        asy_bkgd,asy_total,sigma, asy1, asy2, asy3, amp1, amp2, amp3, phi1, phi2, phi3, field1, field2, lambdaL, lambdaT1, lambdaT2, lambdaT3, delta  = vector
        ...
        return (asy1*internFld_signal1+asy2*internFld_signal2+asy3*internFld_signal3+asy3*KuboToyabe)+asy_bkgd*simpleGss  
...
alpha = 1.07 # variable based on run
field3 = 1
...
labels = ["asy_bkgd","asy_total","sigma","signal_1_asymmetry","signal_2_asymmetry","signal_3_asymmetry","fraction_1","fraction_2","fraction_3","phase_1","phase_2","phase_3","field_1","field_2","field_3","lambdaL","lambdaT_1","lambdaT_2","lambdaT_3","delta"]
ndim = len(labels)
# range estimations
varMin = [0.14,0.06,0.04,0,0,0         ,0,0,0, 0,0,0       ,120,0,0    ,0,0,0,0,0]
varMax = [0.17,0.09,0.2 ,0.01,0.04,0.06,1,1,1, 360,360,360 ,250,100,100   ,5,5,5,5,5]
...
for i in range(len(vector_KT)):
    ...
    print(labels[i]," : ", vector_KT[i],"±",peaks_error_table[i][0][0])

...
fixedLambdaL                = False
# number of signals for zerofield models
signals = 2
# choose if phi is locked to 0 (only for signals = 2)
phi_locked_0 = False
# viewed packing
default                     = 200
...
filenames = ["005807.txt", "005808.txt", "005809.txt", "005810.txt", "005811.txt",
...
# simplification needed to reduce number of variables
phi_zerofield_2f = 0
...
def model3(t,beta,H,phi,lambdaT,sigma):
    lambdaL=0
    simpleGss=np.exp(-1/2*(sigma*10**6*t)**2)
    besselFunction = special.jv(0,2*np.pi*gamma*H*t+np.pi*phi/180)
    return sampleAsymmetry*(beta*besselFunction*np.exp(-lambdaT*10**6*t)+(1-beta)*np.exp(-lambdaL*10**6*t))+GaAsOnlyAsymmetry*simpleGss
...
def model4(t,beta,H,phi,lambdaT,sigma):
    ...
    return sampleAsymmetry*(beta*cosFunction*np.exp(-lambdaT*10**6*t)+(1-beta)*np.exp(-lambdaL*10**6*t))+GaAsOnlyAsymmetry*simpleGss

# model 5 is for zero field with the bessel function with lambdaL free
def model5(t,beta,H,phi,lambdaT,lambdaL,sigma):
    simpleGss=np.exp(-1/2*(sigma*10**6*t)**2)
    besselFunction = special.jv(0,2*np.pi*gamma*H*t+np.pi*phi/180)
    return sampleAsymmetry*(beta*besselFunction*np.exp(-lambdaT*10**6*t)+(1-beta)*np.exp(-lambdaL*10**6*t))+GaAsOnlyAsymmetry*simpleGss
    
# model 6 is for zero field with the relaxed cosin function with lambdaL free
...
# model 7 is for zero field with bessel function, phi = 0
def model7(t,asy1Guess, asyBkgd, beta1, beta2, field1, field2, sigma, lambdaL, lambdaT1, lambdaT2):

    
    asy2 = total_asymmetry - asy1Guess
    asy1 = total_asymmetry - asy2
    simpleGss=np.exp(-1/2*(sigma*10**6*t)**2)
    besselFunction1 = special.jv(0,2*np.pi*gamma*field1*t*10**6+np.pi*phi_zerofield_2f/180)
    besselFunction2 = special.jv(0,2*np.pi*gamma*field2*t*10**6+np.pi*phi_zerofield_2f/180)
    internBsl_signal1 = (beta1*besselFunction1*np.exp(-lambdaT1*10**6*t)+(1-beta1)*np.exp(-lambdaL*10**6*t))
    internBsl_signal2 = (beta2*besselFunction2*np.exp(-lambdaT2*10**6*t)+(1-beta2)*np.exp(-lambdaL*10**6*t))
    
    return asy1*internBsl_signal1+asy2*internBsl_signal2+asyBkgd*simpleGss 

def model8(t,asy1Guess, asyBkgd, beta1, beta2, field1, field2, sigma, lambdaL, lambdaT1, lambdaT2):

    asy2 = total_asymmetry - asy1Guess
    asy1 = total_asymmetry - asy2
    
    simpleGss=np.exp(-1/2*(sigma*10**-6*t)**2)
    cosFunction1 = np.cos(2*np.pi*gamma*field1*t*10**6+np.pi*phi_zerofield_2f/180)
    cosFunction2 = np.cos(2*np.pi*gamma*field2*t*10**6+np.pi*phi_zerofield_2f/180)

    internBsl_signal1 = (beta1*cosFunction1*np.exp(-lambdaT1*10**6*t)+(1-beta1)*np.exp(-lambdaL*10**6*t))
    internBsl_signal2 = (beta2*cosFunction2*np.exp(-lambdaT2*10**6*t)+(1-beta2)*np.exp(-lambdaL*10**6*t))
    
    return asy1*internBsl_signal1+asy2*internBsl_signal2+asyBkgd*simpleGss

def model9(t,asy1Guess, asyBkgd, beta1, beta2, field1, field2, phi1, phi2, sigma, lambdaL, lambdaT1, lambdaT2):

    asy2 = total_asymmetry - asy1Guess
    asy1 = total_asymmetry - asy2
    
    simpleGss=np.exp(-1/2*(sigma*10**-6*t)**2)
    cosFunction1 = np.cos(2*np.pi*gamma*field1*t*10**6+np.pi*phi1/180)
    cosFunction2 = np.cos(2*np.pi*gamma*field2*t*10**6+np.pi*phi2/180)
...
def model10(t,asy1Guess, asyBkgd, beta1, beta2, field1, field2, phi1, phi2, sigma, lambdaL, lambdaT1, lambdaT2):
...
def fitAsymmetryNoField(goodBinT,goodBinA,goodErrorBinA,filename,temperature,withBessel=True):
    ...
    if fixedLambdaL:
        if withBessel:
            ...
            m = Minuit(least_squares,beta=0.05,H=140,phi=0,lambdaT=0.4,sigma=0.6)
        if not withBessel:
            least_squares = LeastSquares(goodBinT,goodBinA,goodErrorBinA,model4)
            m = Minuit(least_squares,beta=0.05,H=140,phi=0,lambdaT=0.4,sigma=0.6)
        m.limits = [(0,1),(0,250),(-180,180),(0,5),(0,5)]
    if not fixedLambdaL:
        if withBessel and signals == 1:
            least_squares = LeastSquares(goodBinT,goodBinA,goodErrorBinA,model5)
            m = Minuit(least_squares,beta=0.05,H=140,phi=0,lambdaT=0.4,lambdaL=0.4,sigma=0.6)
            m.limits = [(0,1),(0,250),(-180,180),(0,5),(0,5),(0,5)]
        if not withBessel and signals == 1:
            least_squares = LeastSquares(goodBinT,goodBinA,goodErrorBinA,model6)
            m = Minuit(least_squares,beta=0.05,H=140,phi=0,lambdaT=0.4,lambdaL=0.4,sigma=0.6)
            m.limits = [(0,1),(0,250),(-180,180),(0,5),(0,5),(0,5)]
        if phi_locked_0 and signals == 2:
            if withBessel:
                least_squares = LeastSquares(goodBinT,goodBinA,goodErrorBinA,model7)
                asy1Guess, asyBkgd, beta1, beta2, field1, field2, sigma, lambdaL, lambdaT1, lambdaT2 = 0.015,0.161,0.194,0.567,30.65,9.89,0.044,0.061,0.323,0.1
                m = Minuit(least_squares,asy1Guess, asyBkgd, beta1, beta2, field1, field2, sigma, lambdaL, lambdaT1, lambdaT2)
            if not withBessel:
                least_squares = LeastSquares(goodBinT,goodBinA,goodErrorBinA,model8)
                asy1Guess, asyBkgd, beta1, beta2, field1, field2, sigma, lambdaL, lambdaT1, lambdaT2 =0.016,0.159,0.178,0.584,30.09,9.957,0.035,0.058,0.398,0.03
                m = Minuit(least_squares,asy1Guess, asyBkgd, beta1, beta2, field1, field2, sigma, lambdaL, lambdaT1, lambdaT2)
            m.limits = [(0,0.09),(GaAsOnlyAsymmetry-0.001,GaAsOnlyAsymmetry+0.001),(0,1),(0,1),(0,250),(0,250),(0,5),(0,5),(0,5),(0,5)]
        if not phi_locked_0 and signals == 2:
            if withBessel:
                least_squares = LeastSquares(goodBinT,goodBinA,goodErrorBinA,model9)
                asy1Guess, asyBkgd, beta1, beta2, field1, field2, phi1, phi2, sigma, lambdaL, lambdaT1, lambdaT2 = 0.015,0.161,0.194,0.567,30.65,9.89,0,0,0.044,0.061,0.323,0.1
                m = Minuit(least_squares,asy1Guess, asyBkgd, beta1, beta2, field1, field2, phi1, phi2, sigma, lambdaL, lambdaT1, lambdaT2)
            if not withBessel:
                least_squares = LeastSquares(goodBinT,goodBinA,goodErrorBinA,model10)
                asy1Guess, asyBkgd, beta1, beta2, field1, field2, phi1, phi2, sigma, lambdaL, lambdaT1, lambdaT2 = 0.015,0.161,0.194,0.567,30.65,9.89,0,0,0.044,0.061,0.323,0.1
                m = Minuit(least_squares,asy1Guess, asyBkgd, beta1, beta2, field1, field2, phi1, phi2, sigma, lambdaL, lambdaT1, lambdaT2)
            m.limits = [(0,0.09),(GaAsOnlyAsymmetry-0.001,GaAsOnlyAsymmetry+0.001),(0,1),(0,1),(0,250),(0,250),(-180,180),(-180,180),(0,5),(0,5),(0,5),(0,5)]
    ...
    if showAll == True or showNoField == True:
        ...
        if not fixedLambdaL:
            ...
            if not withBessel and signals == 1:
                ...
                title = "Asymmetry fit for run " + filename + " with relaxed cosin fit"
            if withBessel and signals == 2 and phi_locked_0 == True:
                plt.plot(viewGoodBinT, model7(viewGoodBinT, *m.values), label="fit",color="orange")
                title = "Asymmetry fit for run " + filename + " with Bessel function fit"
            if not withBessel and signals == 2 and phi_locked_0 == True:
                plt.plot(viewGoodBinT, model8(viewGoodBinT, *m.values), label="fit",color="orange")
                title = "Asymmetry fit for run " + filename + " with relaxed cosin fit"
            if withBessel and signals == 2 and phi_locked_0 == False:
                plt.plot(viewGoodBinT, model9(viewGoodBinT, *m.values), label="fit",color="orange")
                title = "Asymmetry fit for run " + filename + " with Bessel function fit"
            if not withBessel and signals == 2 and phi_locked_0 == False:
                plt.plot(viewGoodBinT, model10(viewGoodBinT, *m.values), label="fit",color="orange")
                title = "Asymmetry fit for run " + filename + " with relaxed cosin fit"
        plt.xlabel("time (s)",fontsize=12)
        plt.ylabel("Asymmetry",fontsize=12)
...
# also return yerr on bins
errorBinA=getErrorBinA(f,b,binRange,binT,binA,filename,temperature,alpha)
#errorBinA = np.var(binA)           
goodBinT, goodBinA, goodErrorBinA = binT, binA, errorBinA

# following instructions on https://iminuit.readthedocs.io/en/stable/tutorial/basic_tutorial.html
fitAsymmetryNoField(goodBinT,goodBinA,goodErrorBinA,filename,temperature,withBessel=True)

fitAsymmetryNoField(goodBinT,goodBinA,goodErrorBinA,filename,temperature,withBessel=False)

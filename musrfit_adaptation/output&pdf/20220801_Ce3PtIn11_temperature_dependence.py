...
def fitAsymmetryNoField(goodBinT,goodBinA,goodErrorBinA,filename,withBessel=True):
    ...
    temperature = dict_runs_temp[filename]  # temperature associated with selected file
    global binT
    # switch back goodBinT values to seconds for fitting
    goodBinT = goodBinT*10**-6
    ...
    if showAll == True or showNoField == True:
        ...
        if withBessel:
            ...
            ax.plot(goodBinT, modelBessel(viewGoodBinT, vector), label="Fit with Bessel \nzero field function",color=colors[1])
            ax.set_xlabel("time (s)",labelpad=10)
            ax.set_ylabel("Asymmetry",labelpad=10)
            ax.ticklabel_format(style="sci", useMathText = True)
...
def fitModel(filename,model):
    ...
    if model == "cos_KT":
        withBessel=False
    return fitAsymmetryNoField(goodBinT,goodBinA,goodErrorBinA,filename,withBessel)
...
# This function uses the chosen_runs and 
parameters, dict_parameters, dict_runs = getDictionaryRuns("cos_KT") 

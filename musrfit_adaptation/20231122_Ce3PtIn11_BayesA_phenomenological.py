...
def plotResults(filename,goodBinT,goodBinA,goodErrorBinA,vector):
    ...
    plt.plot(binT, modelall5(vector), label=str(temperature[i])+"K",color="orange")
...
def posterior_peaks(posterior_pdf,showResults=True):
    ...
    for i in range(len(posterior_pdf)):
        ...
        for k in range(len(peaks)):
            ...
            if showResults:
                plt.axvline(x[peaks][k],0,1,linestyle="--",color="red",label="Peak {0} = ${{{1}}}_{{-{2}}}^{{+{3}}}$".format((k+1), fmt1(qm), fmt(q_minus), fmt(q_plus)))
        ...
        if showResults:
            ...
            #legend+= "\nMusrfit = 0.33(0.06)"
            #plt.axvline(0.33,0,1,linestyle="--",color="orange",label=r"Musrfit = 0.33$\pm$0.06")
...
for filename in filenames:    
    ...
    ranges[5] = [70,150,80]        #fieldCe2
    ranges[6] = [0.,0.015,0.015]      #asyCe1
    ranges[7] = [0.,5,5]            #rlxCe1
    ranges[8] = [30,70,55]          #fieldCe1
...
#%% show relevant figures    
    ...
    #savename = "save_25_" + filename + "_posterior_pdf"
    #savename = "save_5_" +filename+"_posterior_pdf"
    #savename = "save_tightbin10_"+filename+"_posterior_pdf"
    #savename = "save_"+filename+"_posterior_pdf"
    if save:
        # change the list into a numpy object array (not a vector because not all same dimensions)
        posterior = np.array(posterior_pdf_all5,object)
        posterior_pdf_all5 = posterior
    ...
    if filename == "./runfiles/005810.txt" or filename == "./runfiles/005809.txt":
        ...
        peaks_error_table_all5 = temp_table[:]

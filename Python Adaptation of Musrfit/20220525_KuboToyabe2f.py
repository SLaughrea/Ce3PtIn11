...
"""
def model(vector):
        asy1Guess, asy2Guess, delta1, delta2, sigma = vector
        asy1 = asy_total - asy2Guess
        asy2 = asy_total - asy1Guess
        
        KuboToyabe1 = 1/3 + 2/3 * (1-delta1**2*binT)*np.exp(-delta1**2*binT**2/2)
        KuboToyabe2 = 1/3 + 2/3 * (1-delta2**2*binT)*np.exp(-delta2**2*binT**2/2)

        simpleGss=np.exp(-1/2*(sigma*binT)**2)
        return (asy1*KuboToyabe1+asy2*KuboToyabe2)+asy_bkgd*simpleGss  
...
labels = ["signal_1_asymmetry","signal_2_asymmetry","delta_1","delta_2", "sigma"]
ndim = len(labels)
# range estimations
varMin = [0.  ,0.   ,0.,0.,0.]
varMax = [0.01, 0.08,2,2.,1.]

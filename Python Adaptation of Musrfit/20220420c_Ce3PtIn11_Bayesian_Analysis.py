#%%
"""
Bayesian Analysis of 005808.txt by Sébastien Laughrea
"""
...
plt.show()
print(testVector)
initA=(np.random.random(32)-0.5)*arange/100+testVector[0]
initF=(np.random.random(32)-0.5)*frange/100+testVector[1]
initP=(np.random.random(32)-0.5)*prange/100+testVector[2]
initS=(np.random.random(32)-0.5)*srange/100+testVector[3]

pos = np.array([initA,initF,initP,initS]).T
...
MCMCsamples = sampler.get_chain(discard=100,thin=15)
labels = ["total_asymmetry","field", "phi", "sigma"]
for i in range(ndim):
    ax = axes[i]
    ax.plot(MCMCsamples[:, :, i], "k", alpha=0.3)
    ax.set_xlim(0, len(MCMCsamples))
...
print("MCMC posterior mean:", np.mean(MCMCsamples))
print("MCMC posterior std:", np.std(MCMCsamples))

#from IPython.display import display, Math

finalVector = np.array([0.,0.,0.,0.])
for i in range(ndim):

...
alpha = 1.1
# determined from 005807.txt and 005808.txt runs
total_asymmetry = 0.086
...
def model7(t,asy1Guess, asy2Guess, asyBkgd, beta1, beta2, field1, field2, sigma, lambdaL, lambdaT1, lambdaT2):
    ...
    internBsl_signal1 = (beta1*besselFunction1*np.exp(-lambdaT1*t)+(1-beta1)*np.exp(-lambdaL*t))
    internBsl_signal2 = (beta2*besselFunction2*np.exp(-lambdaT2*t)+(1-beta2)*np.exp(-lambdaL*t))
    
    return asy1*internBsl_signal1+asy2*internBsl_signal2+asyBkgd*simpleGss 

def model8(t,asy1Guess, asy2Guess, asyBkgd, beta1, beta2, field1, field2, sigma, lambdaL, lambdaT1, lambdaT2):
    ...
    internBsl_signal1 = (beta1*cosFunction1*np.exp(-lambdaT1*t)+(1-beta1)*np.exp(-lambdaL*t))
    internBsl_signal2 = (beta2*cosFunction2*np.exp(-lambdaT2*t)+(1-beta2)*np.exp(-lambdaL*t))
...
def fitAsymmetryNoField(goodBinT,goodBinA,goodErrorBinA,filename,temperature,withBessel=True):
    ...
    if not fixedLambdaL:
        ...
        if withBessel and signals == 2:
            least_squares = LeastSquares(goodBinT,goodBinA,goodErrorBinA,model7)
            asy1Guess, asy2Guess, asyBkgd, beta1, beta2, field1, field2, sigma, lambdaL, lambdaT1, lambdaT2 = 0.015,0.058,0.161,0.194,0.567,30.65,9.89,0.044,0.061,0.323,0.1
            m = Minuit(least_squares,asy1Guess, asy2Guess, asyBkgd, beta1, beta2, field1, field2, sigma, lambdaL, lambdaT1, lambdaT2)
        if not withBessel and signals == 2:
            least_squares = LeastSquares(goodBinT,goodBinA,goodErrorBinA,model8)
            
            asy1Guess, asy2Guess, asyBkgd, beta1, beta2, field1, field2, sigma, lambdaL, lambdaT1, lambdaT2 =0.016,0.054,0.159,0.178,0.584,30.09,9.957,0.035,0.058,0.398,0.03
            m = Minuit(least_squares,asy1Guess, asy2Guess, asyBkgd, beta1, beta2, field1, field2, sigma, lambdaL, lambdaT1, lambdaT2)

...
def getErrorBinY(f,b,binRange,binX,binY,filename,temperature,alpha):
    ...
    dadf=2*alpha*b/(f*alpha+b)**2
    dadb=-2*alpha*f/(f*alpha+b)**2
...
def fitAsymmetryGaAsSample(goodBinX,goodBinA,goodErrorBinA,filename,temperature):
    ...
    if showAll == True or showEachIterationGaAsSample == True or iteration == 3 and showFinalAsymmetryFit == True:
        ...
        plt.plot(viewGoodBinX, model1(viewGoodBinX, *m.values), label="fit",color="orange")
...
def convergenceGaAsSample(alpha):
    ...
    return iteration, GaAsPlusSampleAsymmetry,alpha,binAmount,errorBinY
...
def convergenceGaAsOnly(alpha):
    ...
    alpha = alpha - 2 * fittedP
    return iteration, GaAsOnlyAsymmetry, alpha, binAmount,errorBinY

def convergence_internFld_internBsl(run,temperature):
    # set correct temperature for this run
    ...
    # also return asymmetry bins positions
    binX,binY=plotBinnedAsymmetry(f,binRange,binAsymmetry,run,temperature)
        
    # also return yerr on bins
    errorBinY=getErrorBinY(f,b,binRange,binX,binY,run,temperature,alpha)
            
    # cut data past 6 
    # cut asymmetry, binned asymmetry (y positions (A) and x positions), and yerr
    goodA, goodBinA, goodBinX, goodErrorBinA = cutData(cut,f,asymmetry,binX,binY,errorBinY,run,temperature)

    # following instructions on https://iminuit.readthedocs.io/en/stable/tutorial/basic_tutorial.html
    fitAsymmetryNoField(goodBinX,goodBinA,goodErrorBinA,run,temperature,withBessel=True)
    fitAsymmetryNoField(goodBinX,goodBinA,goodErrorBinA,run,temperature,withBessel=False)
...
iteration, GaAsPlusSampleAsymmetry, alpha, binAmount,errorBinY = convergenceGaAsSample(alpha)
...
# 1st iteration for convergence of alpha
iteration, GaAsOnlyAsymmetry, alpha, binAmount,errorBinY = convergenceGaAsOnly(alpha)
...
convergence_internFld_internBsl(run="005812.txt",temperature = runParam[5][13:18])

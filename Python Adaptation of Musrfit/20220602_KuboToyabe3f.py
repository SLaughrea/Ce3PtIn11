...
def model(vector):
        asy1Guess, asy2Guess, amp1, amp2, phi1, phi2, field1, field2, lambdaL1, lambdaL2, lambdaT1, lambdaT2, delta, sigma  = vector
        
        asy1 = asy1Guess
        asy2 = asy2Guess
        asy3 = asy_total - asy1Guess - asy2Guess
        ...
        internFld_signal1 = (amp1*cosFunction1*np.exp(-lambdaT1*binT)+(1-amp1)*np.exp(-lambdaL1*binT))
        internFld_signal2 = (amp2*cosFunction2*np.exp(-lambdaT2*binT)+(1-amp2)*np.exp(-lambdaL2*binT))
...
labels = ["signal_1_asymmetry","signal_2_asymmetry","fraction_1","fraction_2","phase_1","phase_2","field_1","field_2","lambdaL_1","lambdaL_2","lambdaT_1","lambdaT_2","delta", "sigma"]
ndim = len(labels)
# range estimations
varMin = [0,0,0,0,0,0,100,0,0,0,0,0,0,0]
varMax = [0.089,0.089,1,1,360,360,150,20,5,5,5,5,5,5]

...
def log_prior(vector):
    a1, a2, aB, aT, b1, b2, f1, f2, sg, ll, t1, t2 = vector
    if (a1min < a1 < a1max and a2min < a2 < a2max and aBmin < aB < aBmax and aTmin < aT < aTmax and b1min < b1 < b1max and \
        b2min < b2 < b2max and f1min < f1 < f1max and f2min < f2 < f2max and \
        sgmin < sg < sgmax and llmin < ll < llmax and t1min < t1 < t1max and t2min < t2 < t2max ):
        return 0
    return -np.inf
    
def prior_transform(vector):
    # convert ndim numbers between 0 and 1 to ndim numbers distributed on their respective range
    return np.array([vector[0]*a1range+a1min,vector[1]*a2range+a2min,vector[2]*aBrange+aBmin,vector[3]*aTrange+aTmin,vector[4]*b1range+b1min, \
                     vector[5]*b2range+b2min,vector[6]*f1range+f1min,vector[7]*f2range+f2min, \
                     vector[8]*sgrange+sgmin,vector[9]*llrange+llmin,vector[10]*t1range+t1min,vector[11]*t2range+t2min])
...
def modelInternFld(vector):
    asy1Guess, asy2Guess, asy_bkgd, total_asymmetry_Guess, beta1, beta2, field1, field2, sigma, lambdaL, lambdaT1, lambdaT2  = vector
    total_asymmetry = total_asymmetry_Guess
    asy1 = total_asymmetry - asy2Guess
    asy2 = total_asymmetry - asy1Guess
    simpleGss=np.exp(-1/2*(sigma*binT)**2)
    cosFunction1 = np.cos(2*np.pi*gamma*field1*binT*10**-6+np.pi*phi/180)
    cosFunction2 = np.cos(2*np.pi*gamma*field2*binT*10**-6+np.pi*phi/180)
    ...
    return asy1*internFld_signal1+asy2*internFld_signal2+asy_bkgd*simpleGss  
...
def modelInternBsl(vector):
    ...
    return asy1*internBsl_signal1+asy2*internBsl_signal2+asy_bkgd*simpleGss 
...
alpha = 1.06 # variable based on run
phi = 0
...
labels = ["signal_1_asymmetry", "signal_2_asymmetry", "bkgd_asymmetry", "total_asymmetry", "amplitude_1", "amplitude_2", "field_1", "field_2","sigma", "lambdaL", "lambdaT_1", "lambdaT_2"]
ndim = len(labels)
# range estimations
a1min=0.;a1max=0.07;a1range=a1max-a1min
a2min=0.;a2max=0.07;a2range=a2max-a2min
aBmin=0.14;aBmax=0.17;aBrange=aBmax-aBmin
aTmin=0.06;aTmax=0.08;aTrange=aTmax-aTmin
...
plotResults("005810.txt",binT,binA,errorBinA,vector_internBsl,withBessel=True)
plotResults("005810.txt",binT,binA,errorBinA,vector_internFld,withBessel=False)

...
def getErrorBinA(f,b):
    ...
    # calculate bins of f and b
        ...
        varB = sum([((x - binB[i]) ** 2) for x in B])
        varF = sum([((x - binF[i]) ** 2) for x in F])
    ...
    dadfsquared = ((alpha*(alpha*beta*binF+binB)-alpha**2*(binF-binB)*beta)/(alpha*beta*binF+binB)**2*errF)**2
    dadbsquared = (-((alpha*beta*binF+binF)/(alpha*beta*binF+binB)**2*alpha)*errB)**2
    errA = np.sqrt(dadfsquared+dadbsquared)
...
def plotResults(filename,goodBinT,goodBinA,goodErrorBinA,vector):
    ...
    plt.plot(binT, modelSD5(vector), label="fit",color="orange")
    plt.errorbar(viewGoodBinT, viewGoodBinA, viewGoodErrorBinA, fmt=".", label="data",color="deepskyblue")
    title = "Asymmetry fit for run " + filename + " with SD5"
...
def log_likelihood_SD5(vector):
    sigma2 = errA ** 2
    return -0.5 * np.sum((binA - modelSD5(vector)) ** 2 /  sigma2  )

def log_probability_SD5(vector):
    lp = log_prior(vector)
    if not np.isfinite(lp):
        return -np.inf
    return lp + log_likelihood_SD5(vector)
...
for filename in filenames:    
    ...
    or filename == "005139.txt" or filename == "005141.txt" or filename == "005140.txt":
        alpha = 0.9632 # 5808_h12.msr
        beta = 1.2276 # 5808_ ?
        total_asymmetry = 0.05355 # 5808_srd.msr
        asysample = total_asymmetry
        asybkgd = 0.1495 # 5807.msr
        rlxAg = 0.0091 #rlxAg found with 5808_srd.msr
    ...
    samplerDyn_SD5 = dynesty.DynamicNestedSampler\
        (log_likelihood_SD5, prior_transform, ndim=ndim,bound="multi",sample="rslice",bootstrap=50)
    #help(samplerDyn_SD5)
    # run with checkpointing
    
    
    samplerDyn_SD5.run_nested()
    
    #samplerDyn_SD5 = NestedSampler.restore('dynesty.save')
    # resume
    #samplerDyn_SD5.run_nested(resume=True)

    sresults_SD5 = samplerDyn_SD5.results
    #samples_SD5, weights_SD5 = sresults_SD5.samples, np.exp(sresults_SD5.logwt - sresults_SD5.logz[-1])
    #mean_SD5, cov_SD5 = dyfunc.mean_and_cov(samples_SD5, weights_SD5)
    
    # plot traces and 1-D marginalized posteriors.
    tfig, taxes, posterior_pdf_SD5 = traceplot(sresults_SD5,show_titles=True)
    tfig.tight_layout()
    plt.show()
    
    # comparing cumulative evidence from different models hints at the model probability
    SD5_cumulative_evidence = sresults_SD5["logz"][-1]
    #print("SD5 log_z posterior evidence = {:}".format(SD5_cumulative_evidence))
    samples_SD5, weights_SD5 = sresults_SD5.samples, np.exp(sresults_SD5.logwt - sresults_SD5.logz[-1])
    
    
    savename = "save_" + filename + "_posterior_pdf"
    #posterior_pdf_SD5 = openSingleFile(savename)
    saveFile(savename,posterior_pdf_SD5)
    
    # comparing cumulative evidence from different models hints at the model probability
    peaks_table_SD5, peaks_error_table_SD5, peaks_rel_height_table_SD5=posterior_peaks(posterior_pdf_SD5,showResults=False)
    
    # get index of peak with highest rel height
    bestPeaks = np.zeros(len(peaks_table_SD5))
    errBestPeaks = np.zeros(len(peaks_table_SD5))

    for i in range(len(peaks_table_SD5)):  
        bestPeaks[i] = peaks_table_SD5[i][np.argmax(peaks_table_SD5[i])]
        #only save positive error
        errBestPeaks[i] = peaks_error_table_SD5[i][np.argmax(peaks_table_SD5[i])][1]

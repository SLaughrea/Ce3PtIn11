...
def plotResults(filename,goodBinT,goodBinA,goodErrorBinA,vector,withBessel=True):
    ...
    if withBessel:
        plt.plot(viewGoodBinT, modelInternBsl(vector), label="fit",color="orange")
        title = "Asymmetry fit for run " + filename + " with Bessel function fit"
    if not withBessel:
        plt.plot(viewGoodBinT, modelInternFld(vector), label="fit",color="orange")
...
def log_prior(vector):
    a1, aB, b1, b2, p1, p2, f1, f2, sg, ll, t1, t2 = vector
    if (a1min < a1 < a1max and aBmin < aB < aBmax and b1min < b1 < b1max and \
        b2min < b2 < b2max and p1min < p1 < p1max and p2min < p2 < p2max and f1min < f1 < f1max and \
        f2min < f2 < f2max and sgmin < sg < sgmax and \
...
def prior_transform(vector):
    ...
    return np.array([vector[0]*a1range+a1min,vector[1]*aBrange+aBmin,vector[2]*b1range+b1min,vector[3]*b2range+b2min, \
                     vector[4]*p1range+p1min,vector[5]*p2range+p2min,vector[6]*f1range+f1min,vector[7]*f2range+f2min,vector[8]*sgrange+sgmin, \
                     vector[9]*llrange+llmin,vector[10]*t1range+t1min,vector[11]*t2range+t2min])
        
def log_likelihood_internFld(vector):
    sigma2 = errorBinA ** 2
    return -0.5 * np.sum((binA - modelInternFld(vector)) ** 2 /  sigma2  )#+ np.log(sigma2))

def log_likelihood_internBsl(vector):
    sigma2 = errorBinA ** 2
    return -0.5 * np.sum((binA - modelInternBsl(vector)) ** 2 /  sigma2  )#+ np.log(sigma2))

def log_probability_internFld(vector):
    lp = log_prior(vector)
    if not np.isfinite(lp):
        return -np.inf
    return lp + log_likelihood_internFld(vector)

def log_probability_internBsl(vector):
    lp = log_prior(vector)
    if not np.isfinite(lp):
        return -np.inf
    return lp + log_likelihood_internBsl(vector)
    
def modelInternFld(vector):
    asy1Guess, asyBkgd, beta1, beta2, phi1, phi2, field1, field2, sigma, lambdaL, lambdaT1, lambdaT2  = vector
    asy1 = asy1Guess
    ...
    internFld_signal1 = (beta1*cosFunction1*np.exp(-lambdaT1*binT)+(1-beta1)*np.exp(-lambdaL*binT))
    internFld_signal2 = (beta2*cosFunction2*np.exp(-lambdaT2*binT)+(1-beta2)*np.exp(-lambdaL*binT))
    return asy1*internFld_signal1+asy2*internFld_signal2+asyBkgd*simpleGss  
    
def modelInternBsl(vector):
...
alpha = 1.08508 # variable based on run
total_asymmetry = 0.09 # variable based on run
...
labels = ["signal_1_asymmetry", "bkgd_asymmetry", "amplitude_1", "amplitude_2", "phi_1", "phi2", "field_1", "field_2", "sigma", "lambdaL", "lambdaT_1", "lambdaT_2"]
ndim = len(labels)
# range estimations
a1min=0.;a1max=0.09;a1range=a1max-a1min
#a2min=0.;a2max=0.09;a2range=a2max-a2min
aBmin=0.16;aBmax=0.16030;aBrange=aBmax-aBmin
b1min=0.;b1max=1.  ;b1range=b1max-b1min
b2min=0.;b2max=1.  ;b2range=b2max-b2min
p1min=-180;p1max=180.;p1range=p1max-p1min
p2min=-180;p2max=180;p2range=p2max-p2min
f1min=0.;f1max=250.;f1range=f1max-f1min
f2min=0.;f2max=250.;f2range=f2max-f2min
sgmin =0.15;sgmax =0.151 ;sgrange=sgmax-sgmin
llmin=0.00001;llmax=0.0001  ;llrange=llmax-llmin
t1min=0.;t1max=5  ;t1range=t1max-t1min
t2min=0.;t2max=5  ;t2range=t2max-t2min
#%% dynesty analysis for both models
# internFld "dynamic" nested sampling
samplerDyn_internFld= dynesty.DynamicNestedSampler(log_likelihood_internFld, prior_transform, ndim=ndim)
samplerDyn_internFld.run_nested()
sresults_internFld = samplerDyn_internFld.results
from dynesty import plotting as dyplot

# summary of the run with dynamic sampling
rfig, raxes = dyplot.runplot(sresults_internFld,logplot=False)
rfig.tight_layout()
# plot traces and 1-D marginalized posteriors.
tfig, taxes, posterior_pdf_internFld = traceplot(sresults_internFld,show_titles=True)
tfig.tight_layout()
# plot the 2-D marginalized posteriors.
cfig, caxes = dyplot.cornerplot(sresults_internFld,show_titles=True,labels=labels)
cfig.tight_layout()
plt.show()

# comparing cumulative evidence from different models hints at the model probability
internFld_cumulative_evidence = sresults_internFld["logz"][-1]
samples_internFld, weights_internFld = sresults_internFld.samples, np.exp(sresults_internFld.logwt - sresults_internFld.logz[-1])
#-------------------------------------------------------------
# internBsl "dynamic" nested sampling
samplerDyn_internBsl= dynesty.DynamicNestedSampler(log_likelihood_internBsl, prior_transform, ndim=ndim)
samplerDyn_internBsl.run_nested()
sresults_internBsl = samplerDyn_internBsl.results
from dynesty import plotting as dyplot

# summary of the run with dynamic sampling
rfig, raxes = dyplot.runplot(sresults_internBsl,logplot=False)
rfig.tight_layout()
# plot traces and 1-D marginalized posteriors.
tfig, taxes, posterior_pdf_internBsl = traceplot(sresults_internBsl,show_titles=True)
tfig.tight_layout()
# plot the 2-D marginalized posteriors.
cfig, caxes = dyplot.cornerplot(sresults_internBsl,show_titles=True,labels=labels)
cfig.tight_layout()
plt.show()

# comparing cumulative evidence from different models hints at the model probability
internBsl_cumulative_evidence = sresults_internBsl["logz"][-1]
samples_internBsl, weights_internBsl = sresults_internBsl.samples, np.exp(sresults_internBsl.logwt - sresults_internBsl.logz[-1])

# comparing cumulative evidence from different models hints at the model probability

print("internBsl log_z posterior evidence = {:}".format(internBsl_cumulative_evidence))
print("internBsl log_z posterior evidence = {:}".format(internFld_cumulative_evidence))
                                          
peaks_table_internFld, peaks_error_table_internFld, peaks_rel_height_table_internFld=posterior_peaks(posterior_pdf_internFld,showResults=True)
peaks_table_internBsl, peaks_error_table_internBsl, peaks_rel_height_table_internBsl=posterior_peaks(posterior_pdf_internBsl,showResults=True)

#%% model comparison
    
#       ln Bij  =           p(Hi/D)             -           p(Hj/D)
ln_bayes_factor = internFld_cumulative_evidence - internBsl_cumulative_evidence
bayes_factor = np.exp(ln_bayes_factor)
print("internBsl log_z posterior evidence = {:}".format(internBsl_cumulative_evidence))
print("internBsl log_z posterior evidence = {:}".format(internFld_cumulative_evidence))
print("ln_bayes_factor                    = ",ln_bayes_factor)
print("bayes_factor                       = ",bayes_factor)

samples_internFld, weights_internFld = sresults_internFld.samples, np.exp(sresults_internFld.logwt - sresults_internFld.logz[-1])
mean_internFld, cov_internFld = dyfunc.mean_and_cov(samples_internFld, weights_internFld)

samples_internBsl, weights_internBsl = sresults_internBsl.samples, np.exp(sresults_internBsl.logwt - sresults_internBsl.logz[-1])
mean_internBsl, cov_internBsl = dyfunc.mean_and_cov(samples_internBsl, weights_internBsl)

vector_internBsl = []
for i in range(len(peaks_table_internBsl)):
    vector_internBsl.append(mean_internBsl[i])
    #indexMax = np.argmax(peaks_rel_height_table_internBsl[i])
    #vector_internBsl.append(peaks_table_internBsl[i][indexMax])

vector_internFld = []
for i in range(len(peaks_table_internFld)):
    vector_internFld.append(mean_internFld[i])
    #indexMax = np.argmax(peaks_rel_height_table_internFld[i])
    #vector_internFld.append(peaks_table_internFld[i][indexMax])

# plot asymmetry using parameters from the weighted means
plotResults(filename,binT,binA,errorBinA,mean_internBsl,withBessel=True)
for i in range(len(mean_internBsl)):
    print(labels[i]," : ", mean_internBsl[i],"±",cov_internBsl[i][i])
plotResults(filename,binT,binA,errorBinA,mean_internFld,withBessel=False)
for i in range(len(mean_internFld)):
    print(labels[i]," : ", mean_internFld[i],"±",cov_internFld[i][i])
"""
The Bayes factor between internFld and internBsl is about 8 e+25, or 
ln(Bayes factor) = 59.64

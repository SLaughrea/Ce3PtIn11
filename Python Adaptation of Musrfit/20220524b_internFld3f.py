...
def plotResults(filename,goodBinT,goodBinA,goodErrorBinA,vector,withBessel=True):
    ...
    if withBessel:
        plt.plot(goodBinT, modelInternBsl(vector), label="fit",color="orange")
        title = "Asymmetry fit for run " + filename + " with Bessel function fit"
    if not withBessel:
        plt.plot(goodBinT, modelInternFld(vector), label="fit",color="orange")
...
def log_probability_internFld(vector):
    for i in range(len(vector)):
        if not varMin[i] < vector[i] < varMax[i]:
            return -np.inf
    return log_likelihood_internFld(vector)

def log_probability_internBsl(vector):
    for i in range(len(vector)):
        if not varMin[i] < vector[i] < varMax[i]:
            return -np.inf
    return log_likelihood_internBsl(vector)
    
def modelInternFld(vector):
    asy1Guess, asy2Guess, asy3Guess, amp1, amp2, amp3, phi1, phi2, phi3, field1, field2, lambdaL, lambdaT1, lambdaT2, lambdaT3  = vector
    asy1 = asy1Guess
    asy2 = asy2Guess
    asy3 = asy3Guess
    ...
    cosFunction3 = np.cos(2*np.pi*gamma*field_3*binT*10**-6+np.pi*phi3/180)
    ...
    return asy1*internFld_signal1+asy2*internFld_signal2+asy3*internFld_signal3+bkgd_asymmetry*simpleGss  
...
def modelInternBsl(vector):
    ...
    besselFunction3 = special.jv(0,2*np.pi*gamma*field_3*binT*10**-6+np.pi*phi3/180)
    internBsl_signal1 = (amp1*besselFunction1*np.exp(-lambdaT1*binT)+(1-amp1)*np.exp(-lambdaL*binT))
    internBsl_signal2 = (amp2*besselFunction2*np.exp(-lambdaT2*binT)+(1-amp2)*np.exp(-lambdaL*binT))
    internBsl_signal3 = (amp3*besselFunction3*np.exp(-lambdaT3*binT)+(1-amp3)*np.exp(-lambdaL*binT))
    return asy1*internBsl_signal1+asy2*internBsl_signal2+asy3*internBsl_signal3+bkgd_asymmetry*simpleGss  
...
total_asymmetry = 0.0734
bkgd_asymmetry = 0.16025
sigma = 0.0542
field_3 = 1
...
labels = ["signal_1_asymmetry", "signal_2_asymmetry", "signal_3_asymmetry",\
          "amplitude_1", "amplitude_2", "amplitude_3", \
          "phase_1","phase_2","phase_3","field_1", "field_2", \
          "lambdaL", "lambdaT_1", "lambdaT_2", "lambdaT_3"]
ndim = len(labels)
# range estimations
varMin = [0.00,0.00,0.00,0.,0.,0.,-180.,-180.,-180.,140.,0.,0.,0.,0.,0.,0.]
varMax = [0.01,0.04,0.02,1.,1.,1.,180.,180.,180.,170.,15.,2.,5.,5.,5.,5.]
varRange = np.zeros_like(varMax)
for i in range(ndim):
    varRange[i] = varMax[i] - varMin[i]
"""
a1min=0.01;a1max=0.02;a1range=a1max-a1min
a2min=0.07;a2max=0.09;a2range=a2max-a2min
aBmin=0.16;aBmax=0.161;aBrange=aBmax-aBmin
b1min=0.4;b1max=1.  ;b1range=b1max-b1min
b2min=0.2;b2max=1.  ;b2range=b2max-b2min
f1min=5.;f1max=160.;f1range=f1max-f1min
f2min=0.;f2max=50.;f2range=f2max-f2min
sgmin=0.;sgmax=0.15;sgrange=sgmax-sgmin
llmin=0.;llmax=5  ;llrange=llmax-llmin
...
print("internFld log_z posterior evidence = {:}".format(internFld_cumulative_evidence))

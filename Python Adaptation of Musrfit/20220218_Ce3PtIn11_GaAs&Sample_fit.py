# CONVERSION OF Ce3PtIn11_06_5807_GaAs_silver.nb by S̩bastien Laughrea

import numpy as np                     # many useful functions in python
#import numpy.ma as ma                 # masked arrays
import matplotlib.pyplot as plt        # plotting
#import matplotlib.colors as mcolors   # consistent colors on plots
from scipy import special              # special functions such as Bessel
...
def getErrorBinY(f,b,binRange,binX,binY,filename,temperature):

    asymmetryLabel = filename + ": T = " + temperature + "K"
    # error on each asymmetry point
    errorA = np.zeros_like(f)
    dadf=-(f-b)/(f+b)**2+1/(f+b)
    dadb=-(f-b)/(f+b)**2-1/(f+b)
...
def model(t,asymmetry1,H,phi,lambda1):
    gamma=0.0135528*10**6
    return asymmetry1*(np.cos(2*np.pi*gamma*H*t+np.pi*phi/180)*np.exp(-lambda1*t))

def fitAsymmetry(goodBinX,goodBinA,goodErrorBinA,filename,temperature):
    ...
    plt.xlabel("time (s)")
    plt.ylabel("Asymmetry")
    title = "Asymmetry fit for run" + filename + " at a temperature of " + temperature + "K"
    plt.title(title)    
    plt.legend(title="\n".join(fit_info));
    plt.show()
...
#%%
"""
These are the calculations to find the asymmetry due to the GaAs and other 
background noises, so everything that isn't the sample.
"""
# clean forward, backward, left and right counts from raw data of a run
f,b,l,r=clean("005813.txt")
    
# we calculate asymmetry
asymmetry = (f-b)/(f+b)
...
# plot positron count, raw asymmetry, binned asymmetry, binned with errorbars
...
errorBinY=getErrorBinY(f,b,binRange,binX,binY,"005807.txt",temperature)
...
GaAsAsymmetry=fitAsymmetry(goodBinX,goodBinA,goodErrorBinA,"005807.txt",temperature)
...
# also return yerr on bins
errorBinY=getErrorBinY(f,b,binRange,binX,binY,"005808.txt",temperature)
...
# following instructions on https://iminuit.readthedocs.io/en/stable/tutorial/basic_tutorial.html
GaAsPlusSampleAsymmetry=fitAsymmetry(goodBinX,goodBinA,goodErrorBinA,"005808.txt",temperature)

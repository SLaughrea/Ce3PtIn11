...
def getErrorBinY(f,b,binRange,binX,binY,filename,temperature,alpha):
    ...
    binB = np.zeros([int(np.round((len(f)/binSize)))+1])
    binF = np.zeros_like(binB)
    ...
    binF=binF[:len(binRange)]
    binB=binB[:len(binRange)]
    
    errB = np.zeros_like(binB)
    errF = np.zeros_like(binB)
    
    for j in range(binAmount):
        errB[j]= np.var(b[binSize*j:binSize*j+binSize])
        errF[j]= np.var(f[binSize*j:binSize*j+binSize])
    
    dadf=-(binF*alpha-binB)/(binF*alpha+binB)**2+1/(binF*alpha+binB)
    dadb=-(binF*alpha-binB)/(binF*alpha+binB)**2-1/(binF*alpha+binB)
    
    
    # propagate uncertainty to calculate variance of binned asymmetry
    errA =1/len(binF)*( errB * np.sum(dadb)**2 + errF * np.sum(dadf)**2)
    ...
    for j in range(len(binY)):
        errorBinY[j]=np.var(errA[(j*binSize):(j*binSize+binSize)])
    """
    errorBinY = errA
    #%%
...
def fitAsymmetryGaAsSample(goodBinX,goodBinA,goodErrorBinA,filename,temperature):
    ...
    # starting values
    m = Minuit(least_squares,asymmetry1=0.22,H=23,phi=0.5,lambda1=0.27,p=0.05)    

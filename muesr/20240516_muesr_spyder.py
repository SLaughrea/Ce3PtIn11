...
def DFTplot(filename,dim,interval0,interval1):
    filename = "./files/density_Ce3PtIn11.txt"
    ...
    frame = pd.read_table(filename, sep='\s+')# *(1.8897259886**3) # to convert to bohr radius units
    ...
    return chosenX,chosenY,chosenZ,chosenF,F

# units in vesta are e/A^3

def plot3d(X,Y,Z,F,a,b,c,cells=[1,1,1],normalized=True,atom_positions=False,filenameDFT = False,dimDFT = False,rangemin=0,rangemax=0,title=False):
    # creating 3d figures
    ...
    index = [z for z in Z if z < cells[2]]
    index = [index for index,value in enumerate(Z) if value < cells[2]]
    X = X[index]
    Y = Y[index]
    Z = Z[index]
    F = F[index]
    ...
    if filenameDFT != False:
        Xden,Yden,Zden,Fden,frame = DFTplot(filenameDFT,dimDFT,rangemin,rangemax)
        # 
        XdenM = Xden[:]
        YdenM = Yden[:]
        ZdenM = Zden[:]
        ...
        for j in range(cells[2]):
            XdenM = np.append(XdenM,Xden)
            YdenM = np.append(YdenM,Yden)
            FdenM = np.append(FdenM,Fden)
            
            ZdenM = np.append(ZdenM,Zden+j)
        ...
        # generate more Xden, Yden, Zden, Fden based on number of cells
        print("Fmax = {}".format(max(Fden)))
        print("Fmin = {}".format(min(Fden)))
        if normalized == False:
            for i in range(len(Fden)):
                ax.plot(Xden[i]*a,Yden[i]*b,Zden[i]*c,color="orange",marker=".",linestyle="None",label = "Electron density",markersize=10,clip_on=False,alpha=(Fden[i]-min(Fden))/(max(Fden)-min(Fden)))
            #ax.scatter(Xden*a,Yden*b,Zden*c,color=colors,label = "Electron density",s=10,clip_on=False)
            #ax.scatter(Xden*a,Yden*b,Zden*c,c=Fden/max(Fden),cmap="viridis",label = "Electron density",s=10,clip_on=False,alpha = Fden/max(Fden))
        if normalized == True:
            #ax.scatter(Xden,Yden,Zden,color=colors,marker = 'o',label = "Electron density",s=10,clip_on=False)
            for i in range(len(Fden)):
                ax.plot(Xden[i],Yden[i],Zden[i],color="orange",marker=".",linestyle="None",label = "Electron density",markersize=10,clip_on=False,alpha=(Fden[i]-min(Fden))/(max(Fden)-min(Fden)))
            #ax.scatter(Xden,Yden,Zden,c=Fden/max(Fden), cmap="viridis",marker = 'o',label = "Electron density",s=10,clip_on=False, alpha = Fden/max(Fden))
    ...
    plt.show()
    return Xden,Yden,Zden,Fden,frame
...
i_q = [[14],[10,11]]
cif_location = "./files/Ce3PtIn11.cif"
...
print(field_at_site([0.5,0.5,0.5],commensurate_type="s",nangles=0))
print(field_at_site([0,0.5,0.27755],commensurate_type="s",nangles=0))
print(field_at_site([0.5,0,0.27755],commensurate_type="s",nangles=0))

print(field_at_site([0.5,0.5,1.5],commensurate_type="s",nangles=0))
print(field_at_site([0.5,0.5,2.5],commensurate_type="s",nangles=0))
print(field_at_site([0.5,0.5,3.5],commensurate_type="s",nangles=0))
print(field_at_site([0.5,0.5,4.5],commensurate_type="s",nangles=0))
print(field_at_site([0.5,0.5,5.5],commensurate_type="s",nangles=0))
print(field_at_site([0.5,0.5,6.5],commensurate_type="s",nangles=0))




filename = "./outputs/XYZF_Ce3PtIn11_n20_q1bq2b.csv"
cells=[1,1,6]
...
filenameDFT = "./files/density_Ce3PtIn11.txt"
       
#title=r"Ce$_3$PtIn$_{11}$ Unit Cell $q_1$ = ($\frac{1}{2},\frac{1}{2},\frac{1}{6}$), $q_2$ = ($\frac{1}{2},\frac{1}{2},0$), $m_c$ = 2.54 $\mu_B$, 116 $\pm$5 G"
title=""
rangemin = 3.73
rangemax = 4
X,Y,Z,F = chooseInterval(X,Y,Z,F,116,135)
Xden,Yden,Zden,Fden,frame=plot3d(X,Y,Z,F,a,b,c,cells=cells,normalized=True,atom_positions=[1],filenameDFT=filenameDFT,dimDFT = [73,73,251],rangemin=rangemin,rangemax=rangemax,title=title)
